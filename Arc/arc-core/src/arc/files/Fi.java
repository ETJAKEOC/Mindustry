...
public class Fi implements Comparable<Fi> {
	...
	public File file() {
		if (type == FileType.external)
			return new File(Core.files.getExternalStoragePath(), file.getPath());
		return file;
	}
	...
	public InputStream read() {
		if (type == FileType.classpath || (type == FileType.internal && !file().exists())
			...
			if (input == null)
				throw new ArcRuntimeException("File not found: " + file + " (" + type + ")");
			return input;
		}
		try {
	...
	public OutputStream write(boolean append) {
		if (type == FileType.classpath)
			throw new ArcRuntimeException("Cannot write to a classpath file: " + file);
		if (type == FileType.internal)
			throw new ArcRuntimeException("Cannot write to an internal file: " + file);
		parent().mkdirs();
		try {
	...
	public Fi[] list() {
		if (type == FileType.classpath)
			throw new ArcRuntimeException("Cannot list a classpath directory: " + file);
	...
	public Fi parent() {
		...
		if (parent == null) {
			if (OS.isWindows) {
				return new Fi("", type) {
					final Fi[] children = Seq.with(File.listRoots()).map(Fi::new).toArray(Fi.class);
	...
	public boolean mkdirs() {
		if (type == FileType.classpath)
			throw new ArcRuntimeException("Cannot mkdirs with a classpath file: " + file);
		if (type == FileType.internal)
	...
	public boolean delete() {
		if (type == FileType.classpath)
			throw new ArcRuntimeException("Cannot delete a classpath file: " + file);
		if (type == FileType.internal)
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof Fi other)) return false;
