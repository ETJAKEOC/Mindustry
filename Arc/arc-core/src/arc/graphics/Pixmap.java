...
import arc.graphics.PixmapIO.PngReader;
...
import arc.util.Java16Buffers;
import arc.util.OS;
import arc.util.UnsafeBuffers;
...
public class Pixmap implements Disposable {
	...
	public Pixmap flipY() {
		...
		for (int y = 0; y < height; y++) {
			int srcPos = y * stride;
			int destPos = (height - 1 - y) * stride;
			copyMem(pixels, srcPos, copy.pixels, destPos, stride);
		}
	...
	private void load(byte[] encodedData, int offset, int len, String file) {
		...
		if (ArcNativesLoader.loaded) {
			try {
				...
				if (pixels == null)
					throw new ArcRuntimeException("Error loading pixmap from image data: " + getFailureReason() + (file == null ? "" : " (" + file + ")"));
