...
import arc.graphics.g2d.PixmapPacker.SkylineStrategy.SkylinePage.Row;
...
public class PixmapPacker implements Disposable {
	...
	public static class GuillotineStrategy implements PackStrategy {
		...
		public Page pack(PixmapPacker packer, String name, Rect rect) {
			...
			if (node == null) {
				if (!packer.allowMultiplePages)
					throw new ArcRuntimeException("Failed to fit sprites into one page");
			...
			if (node == null)
				throw new ArcRuntimeException("Failed to pack sprite '" + name + "' of size " + (int) rect.width + "x" + (int) rect.height + " into page of size " + (int) page.root.rect.width + "x" + (int) page.root.rect.height);
			node.full = true;
