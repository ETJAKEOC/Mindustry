...
public class QuadTree<T extends QuadTreeObject> {
	...
	public T find(float x, float y, float width, float height, Boolf<T> out) {
		if (!leaf) {
			T result;
			if (topLeft.bounds.overlaps(x, y, width, height) && (result = topLeft.find(x, y, width, height, out)) != null)
				return result;
			if (topRight.bounds.overlaps(x, y, width, height) && (result = topRight.find(x, y, width, height, out)) != null)
				return result;
			if (botLeft.bounds.overlaps(x, y, width, height) && (result = botLeft.find(x, y, width, height, out)) != null)
				return result;
			if (botRight.bounds.overlaps(x, y, width, height) && (result = botRight.find(x, y, width, height, out)) != null)
				return result;
		}
