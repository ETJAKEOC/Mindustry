...
public abstract class Group extends Element implements Cullable {
	...
	public Element hit(float x, float y, boolean touchable) {
		...
		for (int i = children.size - 1; i >= 0; i--) {
			...
			if (!child.visible || (child.cullable && cullingArea != null && !cullingArea.overlaps(child.x + child.translation.x, child.y + child.translation.y, child.width, child.height)))
				continue;
