...
import static arc.backend.sdl.jni.SDL.SDL_CreateRGBSurfaceFrom;
import static arc.backend.sdl.jni.SDL.SDL_CreateWindow;
import static arc.backend.sdl.jni.SDL.SDL_DestroyWindow;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_KEYBOARD;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_MOUSE_BUTTON;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_MOUSE_MOTION;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_MOUSE_WHEEL;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_QUIT;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_TEXT_EDIT;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_TEXT_INPUT;
import static arc.backend.sdl.jni.SDL.SDL_EVENT_WINDOW;
import static arc.backend.sdl.jni.SDL.SDL_FreeSurface;
import static arc.backend.sdl.jni.SDL.SDL_GL_BLUE_SIZE;
import static arc.backend.sdl.jni.SDL.SDL_GL_CONTEXT_MAJOR_VERSION;
import static arc.backend.sdl.jni.SDL.SDL_GL_CONTEXT_MINOR_VERSION;
import static arc.backend.sdl.jni.SDL.SDL_GL_CONTEXT_PROFILE_COMPATIBILITY;
import static arc.backend.sdl.jni.SDL.SDL_GL_CONTEXT_PROFILE_CORE;
import static arc.backend.sdl.jni.SDL.SDL_GL_CONTEXT_PROFILE_MASK;
import static arc.backend.sdl.jni.SDL.SDL_GL_CreateContext;
import static arc.backend.sdl.jni.SDL.SDL_GL_DEPTH_SIZE;
import static arc.backend.sdl.jni.SDL.SDL_GL_DOUBLEBUFFER;
import static arc.backend.sdl.jni.SDL.SDL_GL_GREEN_SIZE;
import static arc.backend.sdl.jni.SDL.SDL_GL_MULTISAMPLEBUFFERS;
import static arc.backend.sdl.jni.SDL.SDL_GL_MULTISAMPLESAMPLES;
import static arc.backend.sdl.jni.SDL.SDL_GL_RED_SIZE;
import static arc.backend.sdl.jni.SDL.SDL_GL_STENCIL_SIZE;
import static arc.backend.sdl.jni.SDL.SDL_GL_SetAttribute;
import static arc.backend.sdl.jni.SDL.SDL_GL_SetSwapInterval;
import static arc.backend.sdl.jni.SDL.SDL_GL_SwapWindow;
import static arc.backend.sdl.jni.SDL.SDL_GetClipboardText;
import static arc.backend.sdl.jni.SDL.SDL_GetError;
import static arc.backend.sdl.jni.SDL.SDL_GetVersion;
import static arc.backend.sdl.jni.SDL.SDL_INIT_EVENTS;
import static arc.backend.sdl.jni.SDL.SDL_INIT_VIDEO;
import static arc.backend.sdl.jni.SDL.SDL_Init;
import static arc.backend.sdl.jni.SDL.SDL_PollEvent;
import static arc.backend.sdl.jni.SDL.SDL_Quit;
import static arc.backend.sdl.jni.SDL.SDL_SetClipboardText;
import static arc.backend.sdl.jni.SDL.SDL_SetHint;
import static arc.backend.sdl.jni.SDL.SDL_SetTextInputRect;
import static arc.backend.sdl.jni.SDL.SDL_SetWindowIcon;
import static arc.backend.sdl.jni.SDL.SDL_StartTextInput;
import static arc.backend.sdl.jni.SDL.SDL_StopTextInput;
import static arc.backend.sdl.jni.SDL.SDL_WINDOWEVENT_FOCUS_GAINED;
import static arc.backend.sdl.jni.SDL.SDL_WINDOWEVENT_FOCUS_LOST;
import static arc.backend.sdl.jni.SDL.SDL_WINDOWEVENT_SIZE_CHANGED;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_BORDERLESS;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_FULLSCREEN;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_MAXIMIZED;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_OPENGL;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_RESIZABLE;
import static arc.backend.sdl.jni.SDL.SDL_WINDOW_SHOWN;
...
public class SdlApplication implements Application {
	...
	private void addTextInputListener() {
		addListener(new ApplicationListener() {
			...
			@Override
			public void update() {
				if (Core.scene != null && Core.scene.getKeyboardFocus() instanceof TextField next) {
					if (lastFocus == null) {
						SDL_StartTextInput();
					}
					lastFocus = next;
			...
			if (!OS.hasEnv("JAVA_STARTED_ON_FIRST_THREAD_" + id) || OS.env("JAVA_STARTED_ON_FIRST_THREAD_" + id).equals("0")) { //check if equal to 0 just in case
				...
				try {
					...
					System.exit(proc.waitFor());
				         ...
				         URISyntaxException e) { //some part of this failed, likely failed to find java
					Log.err(e);
