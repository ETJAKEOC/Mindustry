...
import static arc.backend.sdl.jni.SDL.SDL_BUTTON_LEFT;
import static arc.backend.sdl.jni.SDL.SDL_BUTTON_MIDDLE;
import static arc.backend.sdl.jni.SDL.SDL_BUTTON_RIGHT;
import static arc.backend.sdl.jni.SDL.SDL_BUTTON_X1;
import static arc.backend.sdl.jni.SDL.SDL_BUTTON_X2;
...
public class SdlInput extends Input {
	...
	private final byte[] strcpy = new byte[32];
	private final Seq<EditEvent> stringEditEvents = new Seq<>();
	...
	void handleFieldCandidate(EditEvent e) {
		...
		String text = e.text;
		if (Core.scene != null && Core.scene.getKeyboardFocus() instanceof TextField field) {

			if (field.imeData instanceof ImeData data) {
