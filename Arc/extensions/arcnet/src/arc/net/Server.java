...
public class Server implements EndPoint {
	...
	private final ConcurrentHashMap<InetSocketAddress, Connection> udpAddressToConnection = new ConcurrentHashMap<>();
	...
	private final NetListener dispatchListener = new NetListener() {
		@Override
	...
	public void sendToAllUDP(Object object, Iterable<Connection> connections) {
		...
		synchronized (bulkWriteLock) {
			...
			for (Connection con : connections) {
				if (!con.isConnected())
					continue; //note: since this method accepts a list of connections, there may be stale connections, so filter for that (not possible otherwise)
				try {
					con.sendUDPBuffer(buffer);
				} catch (Exception e) {
					ArcNet.handleError(e);
