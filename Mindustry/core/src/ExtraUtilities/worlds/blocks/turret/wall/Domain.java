...
public class Domain extends Block {
	...
	public void setStats() {
		...
		if (healByPercent)
			...
			stats.add(Stat.repairTime, Core.bundle.format("stat-heal-unit", reloadHU / 60f, healAmountUnit));
	...
	public class DomainBuild extends Building implements Ranged {
		...
		public float updateSpeeder() {
			...
			indexer.eachBlock(this, range * efficiency, other -> other != this && other.block != null && other.block.canOverdrive, other -> other.applyBoost(realSpeed, reloadS + 1));
		...
		public void draw() {
			super.draw();
			if (buildup > 0f) {
				color(Color.valueOf("bf92f9").a(buildup / shieldHealth * 0.8f));
