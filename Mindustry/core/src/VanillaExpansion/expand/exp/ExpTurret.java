...
public class ExpTurret extends PowerTurret {
	...
	public void init() {
		...
		if (damageReduction == null) damageReduction = new EField.EExpoZero(f -> {
		}, 0.1f, Mathf.pow(4f + size, 1f / maxLevel), true, null, v -> Strings.autoFixed(Mathf.roundPositive(v * 10000) / 100f, 2) + "%");
	}
	...
	public void drawPlace(int x, int y, int rotation, boolean valid) {
		...
		if (rangeStart != rangeEnd)
			Drawf.dashCircle(x * tilesize + offset, y * tilesize + offset, rangeEnd, VEPal.exp);
	...
	public boolean canPlaceOn(Tile tile, Team team, int rotation) {
		...
		tile.getLinkedTilesAs(this, inside -> {
			if (inside.build == null || seqs.contains(inside.build) || seqs.size > 1)
				return; //no point of checking if there are already two in seqs
			if (inside.block() == pregrade && ((ExpTurretBuild) inside.build).level() >= pregradeLevel)
