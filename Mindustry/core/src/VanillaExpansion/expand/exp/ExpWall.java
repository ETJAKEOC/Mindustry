	...
	public class ExpWallBuild extends ExpBaseBuild {
		...
		public void draw() {
			...
			if (top != region) {
				...
				if (!state.isPaused() && updateEffect != Fx.none && top == levelRegions[levelRegions.length - 1] && Mathf.chanceDelta(updateChance))
					updateEffect.at(x + Mathf.range(size * 4f), y + Mathf.range(size * 4f), VEPal.exp);
			}
		}
