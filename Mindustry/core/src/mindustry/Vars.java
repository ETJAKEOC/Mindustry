...
import mindustry.mod.DataAssetCache;
...
public class Vars implements Loadable{
    ...
    public static ExecutorService mainExecutor = executor;
    ...
    public static void init(){
        ...
        if(loadLocales){
            String[] stra = files.internal("locales").readString().split("\n");
        ...
        javaPath =
            ...
            files.local("jre/bin/java").exists() ? files.local("jre/bin/java").absolutePath() : // Unix
            files.local("jre/bin/java.exe").exists() ? files.local("jre/bin/java.exe").absolutePath() : // Windows
            "java";

        state = new GameState();

        mobile = app.isMobile() || testMobile;
        ios = app.isIOS();
    ...
    public static void finishLaunch(){
        settings.put("lastBuild", Version.build);
        settings.put("lastBuildString", Version.buildString());
    ...
    public static void loadSettings(){
        ...
        if(steam || Version.isSteam){
            settings.setDataDirectory(files.local("saves/"));
        }
        ...
        try{
            ...
            if(!headless){
                Time.run(10f, () -> ui.showInfo(bundle.format("bundle.external", handle.absolutePath())));
            }
        }catch(Throwable e){
            //no external bundle found

            Fi handle = files.internal("bundles/bundle");
            Locale locale;
            ...
            if(locale.toString().equals("router")){
                I18NBundle defBundle = I18NBundle.createBundle(files.internal("bundles/bundle"));
