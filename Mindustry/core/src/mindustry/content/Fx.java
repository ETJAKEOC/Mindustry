...
import static arc.graphics.g2d.Lines.poly;
...
public class Fx {
	...
	bulletSparkSmokeTrailSmall = new Effect(28f, e -> {
		...
		for (int s = 0; s < rand.random(0, 2); s++) {
	...
	arcShieldBreak = new Effect(40, e -> {
		...
		if (e.data instanceof Unit u) {
			...
			if (ab != null) {
				...
				for (int i : Mathf.signs) {
					float
							px = pos.x + trnsx(u.rotation + ab.angleOffset - ab.angle / 2f * i, ab.radius + ab.width / 2),
							py = pos.y + trnsy(u.rotation + ab.angleOffset - ab.angle / 2f * i, ab.radius + ab.width / 2),
							px1 = pos.x + trnsx(u.rotation + ab.angleOffset - ab.angle / 2f * i, ab.radius - ab.width / 2),
							py1 = pos.y + trnsy(u.rotation + ab.angleOffset - ab.angle / 2f * i, ab.radius - ab.width / 2);
					line(px, py, px1, py1);
				}
			}
		}
