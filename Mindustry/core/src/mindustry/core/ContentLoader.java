...
import VanillaExpansion.content.VEBlocks;
import VanillaExpansion.content.VEEnvironBlocks;
import VanillaExpansion.content.VEItems;
import VanillaExpansion.content.VELiquids;
import VanillaExpansion.content.VEPlanets;
import VanillaExpansion.content.VETechTree;
import VanillaExpansion.content.VEUnitTypes;
import arc.Events;
import arc.files.Fi;
...
public class ContentLoader {
	...
	private final ObjectMap<String, MappableContent> nameMap = new ObjectMap<>();
	...
	public void createBaseContent() {
		...
		// Integrated Mod Content - Vanilla Expansion
		LoadedMod veMod = new LoadedMod(
			new Fi(""), new Fi(""), null, null,
			new Mods.ModMeta(){{ name = "ve"; displayName = "Vanilla Expansion"; }}
		);
		setCurrentMod(veMod);
		VEItems.load();
		VELiquids.load();
		...
		// Integrated Mod Content - Extra Utilities
		LoadedMod euMod = new LoadedMod(
			new Fi(""), new Fi(""), null, null,
			new Mods.ModMeta(){{ name = "extra-utilities"; displayName = "Extra Utilities"; }}
		);
		setCurrentMod(euMod);
		EUGet.loadItems();
		...
		// Integrated Mod Content - New Horizon
		LoadedMod nhMod = new LoadedMod(
			new Fi(""), new Fi(""), null, null,
			new Mods.ModMeta(){{ name = "new-horizon"; displayName = "New Horizon"; }}
		);
		NewHorizon.MOD = nhMod;
		setCurrentMod(nhMod);
		NHVars.init();
		EntityRegister.load();
		NHRegister.load();
