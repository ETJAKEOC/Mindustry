...
import static mindustry.Vars.maxPlayerPreviewPlans;
...
import mindustry.io.TypeIO.ClientBuildPlans;
...
public class NetServer implements ApplicationListener {
	...
	public TeamAssigner assigner = (player, players) -> {
		if (state.rules.pvp) {
			...
			TeamData re = state.teams.getActive().min(data -> {
				if ((state.rules.waveTeam == data.team && state.rules.waves) || !data.hasCore() || data.team == Team.derelict || !data.team.rules().protectCores)
	...
	private final IntSet buildHealthChanged = new IntSet();
	...
	private final Writes dataStreamWrites = new Writes(dataStream);
	/**
	 ...
	 */
	private final ObjectMap<String, Seq<Cons2<Player, byte[]>>> customBinaryPacketHandlers = new ObjectMap<>();
	/**
	 * Packet handlers for logic client data
	 */
	private final ObjectMap<String, Seq<Cons2<Player, Object>>> logicClientDataHandlers = new ObjectMap<>();
	/**
	 * Reused Seq<Player> for writing entity snapshots per team.
	 */
	private final Seq<Player> playersToSend = new Seq<>(false);
	private final Seq<NetConnection> tempConnections = new Seq<>(false);
	...
	public NetServer() {
		...
		net.handleServer(ConnectPacket.class, (con, packet) -> {
			...
			if (admins.isIPBanned(con.address) || admins.isSubnetBanned(con.address) || con.kicked || !con.isConnected())
				return;
	...
	public static void requestWorld(Player player) {
		if (!player.con.hasBegunConnecting || player.con.determiningAssets || !player.con.receivingAssets || player.con.hasConnected)
			return;
