...
import static arc.scene.actions.Actions.fadeOut;
import static arc.scene.actions.Actions.sequence;
...
import mindustry.ui.fragments.PerformanceFragment;
...
public class UI implements ApplicationListener, Loadable {
	...
	public void showInfoPopup(@Nullable String info, @Nullable String id, float duration, int align, int top, int left, int bottom, int right) {
		...
		table.actions(Actions.delay(duration), Actions.remove(), Actions.run(() -> {
			if (id != null) popups.remove(id);
		}));
	...
	public void showLabel(@Nullable String info, int id, float duration, float worldx, float worldy, int flags) {
		...
		if (id != -1 && label.duration >= 0 && label.expired == null)
			label.expired = () -> labels.remove(id); // only set once to prevent extra garbage for updated labels
		label.add();
	}
