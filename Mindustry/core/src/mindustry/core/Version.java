...
public class Version {
	...
	public static String combined() {
		if (build == -1) {
			return "custom build" + (modifier.equals("release") || modifier.equals("unknown") ? "" : "-" + modifier);
		}
		String ver = "v" + build + (revision == 0 ? "" : "." + revision);
		if (!modifier.equals("release") && !modifier.equals("official") && !modifier.equals("unknown")) {
			ver += "-" + modifier;
		}
		if (!commitHash.equals("unknown")) {
			ver += " (" + commitHash + ")";
		}
		return ver;
	}
