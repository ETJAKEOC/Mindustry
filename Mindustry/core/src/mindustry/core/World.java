...
import static mindustry.Vars.darkRadius;
...
import arc.math.geom.Geometry.Raycaster;
...
import mindustry.game.EventType.TileFloorChangeEvent;
...
public class World {
	...
	private void setSectorRules(Sector sector, boolean saveInfo) {
		...
		for (Tile tile : tiles) {
			...
			if (tile.floor().itemDrop != null && !tile.block().isStatic())
				content.add(tile.floor().itemDrop);
			if (tile.overlay().itemDrop != null && !tile.block().isStatic())
