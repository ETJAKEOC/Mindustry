...
public class MapGenerateDialog extends BaseDialog {
	...
	MirrorFilter hit(float x, float y) {
		...
		for (var filter : filters) {
			if (filter instanceof MirrorFilter mirror) {
		...
		if (amount == 1 && out != null && x >= dragRect.x - pad && y >= dragRect.y - pad && x <= dragRect.x + dragRect.width + pad && y <= dragRect.y + dragRect.height + pad)
			return out;
		return null;
	}
