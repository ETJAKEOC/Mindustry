...
public class MapLocalesDialog extends BaseDialog {
	...
	private final Table langs;
	private final Table main;
	private final Table propView;
	...
	private void setup() {
		...
		cont.table(Tex.button, t -> {
			TextField name = t.field(Core.bundle.get("locales.property.name"), s -> {
			}).maxTextLength(64).fillX().padTop(10f).get();
			t.row();
			TextField value = t.area(Core.bundle.get("locales.property.text"), s -> {
	...
	private void buildMain() {
		...
		main.pane(p -> {
			...
			for (var key : keys) {
				...
				if (!searchString.isEmpty() && !comparsionString.contains(searchString.toLowerCase()))
					continue;
	...
	private void iconsTable(Table table, String search, Dialog dialog, Cons<String> cons) {
		...
		if (codes.size > 0)
			table.image().colspan(cols).growX().width(-1f).height(3f).color(Pal.accent).row();
