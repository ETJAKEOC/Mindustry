...
import arc.util.Tmp;
import mindustry.editor.MapObjectivesCanvas.ObjectiveTilemap.ObjectiveTile.Connector;
import mindustry.editor.MapObjectivesDialog.TypeInfo;
	...
	public class ObjectiveTilemap extends WidgetGroup {
		...
		public void draw() {
			...
			if (connecting != null) {
				...
				if (hit(pos.x, pos.y, true) instanceof Connector con && connecting.canConnectTo(con))
					conTarget = con;
			}
					...
					addCaptureListener(new InputListener() {
						...
						public void touchUp(InputEvent event, float x, float y, int pointer, KeyCode button) {
							...
							if (ObjectiveTilemap.this.hit(pos.x, pos.y, true) instanceof Connector con && con.canConnectTo(Connector.this)) {
								if (findParent) {
									if (!obj.parents.remove(con.tile().obj))
										obj.parents.add(con.tile().obj);
								} else {
									if (!con.tile().obj.parents.remove(obj))
										con.tile().obj.parents.add(obj);
								}
							}
