...
import static mindustry.editor.MapObjectivesCanvas.bounds;
import static mindustry.editor.MapObjectivesCanvas.objHeight;
import static mindustry.editor.MapObjectivesCanvas.objWidth;
...
import mindustry.game.MapObjectives.Alignment;
import mindustry.game.MapObjectives.Immutable;
import mindustry.game.MapObjectives.IndexBool;
import mindustry.game.MapObjectives.LabelFlag;
import mindustry.game.MapObjectives.LogicCode;
import mindustry.game.MapObjectives.MapObjective;
import mindustry.game.MapObjectives.Multiline;
import mindustry.game.MapObjectives.ObjectiveMarker;
import mindustry.game.MapObjectives.Researchable;
import mindustry.game.MapObjectives.Second;
import mindustry.game.MapObjectives.Synthetic;
import mindustry.game.MapObjectives.TextureHolder;
import mindustry.game.MapObjectives.TilePos;
import mindustry.game.MapObjectives.Unordered;
import mindustry.game.MapObjectives.Vertices;
...
public class MapObjectivesDialog extends BaseDialog {
	...
	static {
		...
		for (var obj : MapObjectives.allObjectiveTypes)
			setInterpreter(obj.get().getClass(), defaultInterpreter());
		for (var mark : MapObjectives.allMarkerTypes)
			setInterpreter(mark.get().getClass(), defaultInterpreter());
		...
		setInterpreter(Seq.class, (cont, name, type, field, remover, indexer, get, set) -> cont.table(main -> {
			...
			var header = main.table(Tex.button, t -> {
				...
				if (remover != null)
					t.button(Icon.trash, Styles.emptyi, remover).fill().padRight(4f);
				if (indexer != null) {
	...
	public static class TypeInfo {
		...
		public static Class<?> raw(Type type) {
			...
			if (type instanceof GenericArrayType c)
				return Reflect.newArray(raw(c.getGenericComponentType()), 0).getClass();
