...
import static mindustry.mod.DataImagePacker.regionPrefix;
...
public class MapImagesView implements AssetView {
					...
					t.button("@asset.image.importzip", Icon.download, style, () -> FileChooser.open("zip").submit(file -> {
						...
						ui.loadAnd(() -> {
							try {
								...
								if (imported > 0)
									idiag.cont.add(Core.bundle.format("asset.image.imported", imported)).row();
								if (!errors.isEmpty()) {
	...
	public void build(MapAssetsDialog diag, Table inner) {
		...
		for (var image : getImages()) {
			...
			if (diag.searchString != null && !image.path.toLowerCase().contains(diag.searchString))
				continue;
