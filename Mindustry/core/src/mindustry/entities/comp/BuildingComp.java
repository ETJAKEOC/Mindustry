...
import mindustry.annotations.Annotations.Final;
...
abstract class BuildingComp implements Posc, Teamc, Healthc, Buildingc, Timerc, QuadTreeObject, Displayable, Sized, Senseable, Controllable, Settable, AmbientSource {
	...
	public double sense(LAccess sensor) {
		return switch (sensor) {
			...
			case totalPower ->
					power == null || block.consPower == null ? 0 : power.status * (block.consPower.buffered ? block.consPower.capacity : 1f);
			...
			case controlled ->
					this instanceof ControlBlock c && c.isControlled() ? GlobalVars.ctrlPlayer : 0;
			case payloadCount ->
					(getPayloads() != null ? getPayloads().total() : 0) + (getPayload() != null ? 1 : 0);
	...
	public Object senseObject(LAccess sensor) {
		return switch (sensor) {
			...
			case payloadType ->
					getPayload() instanceof UnitPayload p1 ? p1.unit.type : getPayload() instanceof BuildPayload p2 ? p2.block() : null;
			default -> noSensed;
		};
	}
