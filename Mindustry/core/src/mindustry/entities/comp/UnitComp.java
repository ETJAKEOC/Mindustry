...
import static mindustry.logic.GlobalVars.ctrlCommand;
import static mindustry.logic.GlobalVars.ctrlPlayer;
import static mindustry.logic.GlobalVars.ctrlProcessor;
...
abstract class UnitComp implements Healthc, Physicsc, Hitboxc, Statusc, Teamc, Itemsc, Rotc, Unitc, Weaponsc, Drawc, Syncc, Shieldc, Displayable, Ranged, Minerc, Builderc, Senseable, Settable {
	...
	@Import
	float x, y, rotation, maxHealth, drag, armor, hitSize, health, shield, dragMultiplier, armorOverride, speedMultiplier;
	@Import
	Team team;
	@Import
	int id;
	...
	public double sense(LAccess sensor) {
		return switch (sensor) {
			...
			case cameraX ->
					controller instanceof Player player ? World.conv(player.con == null ? Core.camera.position.x : player.con.viewX) : 0;
			case cameraY ->
					controller instanceof Player player ? World.conv(player.con == null ? Core.camera.position.y : player.con.viewY) : 0;
			case cameraWidth ->
					controller instanceof Player player ? World.conv(player.con == null ? Core.camera.width : player.con.viewWidth) : 0;
			case cameraHeight ->
					controller instanceof Player player ? World.conv(player.con == null ? Core.camera.height : player.con.viewHeight) : 0;
			...
			case pingX ->
					controller instanceof Player p && p.isPinging() ? World.conv(p.pingX) : NaN;
			case pingY ->
					controller instanceof Player p && p.isPinging() ? World.conv(p.pingY) : NaN;
			default -> NaN;
		};
	}
	...
	public Object senseObject(LAccess sensor) {
		return switch (sensor) {
			...
			case controller ->
					!isValid() ? null : controller instanceof LogicAI log ? log.controller : this;
			...
			case building ->
					isBuilding() && !buildPlan().breaking ? buildPlan().tile().build : null;
