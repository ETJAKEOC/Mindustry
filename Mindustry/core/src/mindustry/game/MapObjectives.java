...
import arc.util.Tmp;
import arc.util.serialization.AllowSerialization;
...
public class MapObjectives implements Iterable<MapObjective>, Eachable<MapObjective> {
	...
	public static abstract class MapObjective implements AllowSerialization {
		...
		public @Nullable
		@LogicCode String completionLogicCode;
	...
	public static class FlagObjective extends MapObjective {
		...
		public String text() {
			...
			if (text.startsWith("@")) {
				if (state.mapLocales.containsProperty(text.substring(1)))
					return state.mapLocales.getProperty(text.substring(1));
	...
	public static abstract class ObjectiveMarker implements JsonSerializable {
		...
		public void control(LMarkerControl type, double p1, double p2, double p3) {
			...
			switch (type) {
				case world ->
						state.markers.updateMarker(state.markers.worldMarkers, this, !Mathf.equal((float) p1, 0f), m -> m.world, (m, i) -> m.world = i);
				case minimap ->
						state.markers.updateMarker(state.markers.mapMarkers, this, !Mathf.equal((float) p1, 0f), m -> m.minimap, (m, i) -> m.minimap = i);
				case light ->
						state.markers.updateMarker(state.markers.lightMarkers, this, !Mathf.equal((float) p1, 0f), m -> m.light, (m, i) -> m.light = i);
		...
		public void control(LMarkerControl type, double p1, double p2, double p3) {
			...
			if (!Double.isNaN(p1)) {
				switch (type) {
					...
					case outline ->
							flags = (byte) Pack.bitmask(flags, WorldLabel.flagOutline, !Mathf.equal((float) p1, 0f));
					case labelFlags ->
							flags = (byte) Pack.bitmask(flags, WorldLabel.flagBackground, !Mathf.equal((float) p1, 0f));
			...
			if (!Double.isNaN(p2)) {
				switch (type) {
					case labelFlags ->
							flags = (byte) Pack.bitmask(flags, WorldLabel.flagOutline, !Mathf.equal((float) p2, 0f));
				}
			}
		}
	...
	public static class LineMarker extends PosMarker {
		...
		public void control(LMarkerControl type, double p1, double p2, double p3) {
			...
			if (!Double.isNaN(p1) && !Double.isNaN(p2)) {
				switch (type) {
					case posi ->
							((int) p1 == 0 ? pos : (int) p1 == 1 ? endPos : Tmp.v1).x = (float) p2 * tilesize;
					case colori ->
							((int) p1 == 0 ? color1 : (int) p1 == 1 ? color2 : Tmp.c1).fromDouble(p2);
				}
			}
			...
			if (!Double.isNaN(p1) && !Double.isNaN(p3)) {
				switch (type) {
					case posi ->
							((int) p1 == 0 ? pos : (int) p1 == 1 ? endPos : Tmp.v1).y = (float) p3 * tilesize;
				}
			}
		}
	}
	...
	public static class TextureMarker extends PosMarker {
		...
		public void draw(float scaleFactor) {
			...
			if (Mathf.equal(width, 0f))
				width = fetchedRegion.width * fetchedRegion.scl() * Draw.xscl;
			if (Mathf.equal(height, 0f))
				height = fetchedRegion.height * fetchedRegion.scl() * Draw.yscl;
