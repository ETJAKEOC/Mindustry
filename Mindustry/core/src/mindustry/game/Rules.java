...
public class Rules {
	...
	public void retainContentFields(Rules source) {
		...
		if (Seq.with(source.bannedBlocks).contains(Content::isPatchContent))
			bannedBlocks = source.bannedBlocks;
		if (Seq.with(source.bannedUnits).contains(Content::isPatchContent))
			bannedUnits = source.bannedUnits;
