...
import static mindustry.Vars.maxLoadoutSchematicPad;
...
import mindustry.input.InputHandler.QueryEachable;
import mindustry.input.Placement;
import mindustry.input.Placement.NormalizeResult;
import mindustry.io.JsonIO;
import mindustry.io.SaveFileReader;
import mindustry.io.TypeIO;
import mindustry.io.TypeIO.ContentMapper;
...
public class Schematics implements Loadable {
	...
	private final ObjectMap<CoreBlock, Schematic> defaultLoadouts = new ObjectMap<>();
	...
	private void checkLoadout(Schematic s, boolean customSchem) {
		...
		if ((customSchem && (s.width > maxSize || s.height > maxSize
				|| s.tiles.contains(t -> t.block.buildVisibility == BuildVisibility.sandboxOnly || !t.block.unlocked()) || cores > 1)))
			return;
