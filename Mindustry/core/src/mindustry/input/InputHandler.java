...
import static mindustry.Vars.maxPingTextLength;
...
public abstract class InputHandler implements InputProcessor, GestureListener {
	...
	private final Seq<BuildPlan> plansOut = new Seq<>(BuildPlan.class);
	...
	public static void transferInventory(Player player, Building build) {
		if (player == null || build == null || !player.within(build, itemTransferRange) || build.items == null || player.dead() || !build.allowDeposit())
			return;
	...
	public void update() {
		...
		if (logicCutscene && !renderer.isCutscene()) {
			camera.position.lerpDelta(logicCamPan, logicCamSpeed);
		} else {
	...
	public void drawOtherBuildPlans() {
		...
		Groups.player.each(player -> {
			...
			player.previewPlanTree.intersect(Tmp.r1.grow(tilesize * 2f), plan -> {
				if (plan.block == null || plan.isDone() || (current != null && player.x == current.x && player.y == current.y && player.unit().activelyBuilding()))
					return;
	...
	public void drawBuildPlans() {
		...
		for (BuildPlan plan : plansOut) {
			if (plan.progress > 0.01f || (current == plan && plan.initialized && (u.within(plan.x * tilesize, plan.y * tilesize, u.type.buildRange) || state.isEditor())))
				continue;
		...
		for (BuildPlan plan : u.plans) {
			if (plan.progress > 0.01f || plan.breaking || (current == plan && plan.initialized && (u.within(plan.x * tilesize, plan.y * tilesize, u.type.buildRange) || state.isEditor())))
				continue;
	...
	int rawTileX() {
		return World.toTile(input.mouseWorld().x);
	}

	int rawTileY() {
		return World.toTile(input.mouseWorld().y);
	}

	int tileX(float cursorX) {
		Vec2 vec = input.mouseWorld(cursorX, 0);
		if (selectedBlock()) {
	...
	int tileY(float cursorY) {
		Vec2 vec = input.mouseWorld(0, cursorY);
		if (selectedBlock()) {
			vec.sub(block.offset, block.offset);
		}
	...
	public float mouseAngle(float x, float y) {
		return input.mouseWorld(getMouseX(), getMouseY()).sub(x, y).angle();
	}

	public @Nullable Unit selectedUnit() {
		Unit unit = Units.closest(player.team(), input.mouseWorld().x, input.mouseWorld().y, 40f, u -> u.isAI() && u.playerControllable());
		if (unit != null) {
			unit.hitbox(Tmp.r1);
			Tmp.r1.grow(6f);
			if (Tmp.r1.contains(input.mouseWorld())) {
				return unit;
			}
		}

		Building build = world.buildWorld(input.mouseWorld().x, input.mouseWorld().y);
	...
	public void add() {
		input.getInputProcessors().remove(i -> i instanceof InputHandler || (i instanceof GestureDetector && ((GestureDetector) i).getListener() instanceof InputHandler));
		input.addProcessor(detector = new GestureDetector(20, 0.5f, 0.3f, 0.15f, this));
	...
	public boolean canDropItem() {
		return droppingItem && !canTapPlayer(input.mouseWorldX(), input.mouseWorldY());
	}
	...
	void iterateLine(int startX, int startY, int endX, int endY, Cons<PlaceLine> cons) {
		Seq<Point2> points;
		boolean diagonal = input.keyDown(Binding.diagonalPlacement);
