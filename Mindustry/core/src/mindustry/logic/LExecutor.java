...
import static mindustry.Vars.logicItemTransferRange;
...
public class LExecutor {
	...
	public static void runLogicScript(@Nullable String code, int maxInstructions, boolean loop) {
		...
		for (int i = 1; i < maxInstructions; i++) {
			if ((!loop && executor.counter.numval >= executor.instructions.length || executor.counter.numval < 0) || executor.yield)
				break;
			executor.runOnce();
		}
	}
		...
		public void run(LExecutor exec) {
			...
			if (unitObj instanceof Unit unit && (ai != null || !control)) {
				...
				switch (type) {
					...
					case itemDrop -> {
						if (p1.obj() != Blocks.air && !exec.timeoutDone(unit, LogicAI.transferDelay))
							return;
	...
	public static class QueryI implements LInstruction {
		...
		public void run(LExecutor exec) {
			...
			if (type == QueryType.bullet)
				return; //TODO: bugged due to bullets being pooled, need a way to have references to them cleaned up after death.
			...
			if (circle) {
				switch (type) {
					...
					case unit ->
							results.<Unit>as().removeAll(u -> !u.within(circleX, circleY, radius + u.hitSize / 2f));
					case bullet ->
							results.<Bullet>as().removeAll(u -> !u.within(circleX, circleY, radius + u.hitSize / 2f));
					case building ->
							results.<Building>as().removeAll(u -> !u.within(circleX, circleY, radius + u.hitSize() / 2f));
				}
			}
		}
	}
			...
			switch (rule) {
				...
				case ban -> {
					...
					if (cont instanceof Block b) {
						...
						if (state.rules.bannedBlocks.add(b) && !headless)
				...
				case unban -> {
					Object cont = value.obj();
					if (cont instanceof Block b) {
						if (state.rules.bannedBlocks.remove(b) && !headless)
							ui.hudfrag.blockfrag.rebuild();
					} else if (cont instanceof UnitType u) {
						state.rules.bannedUnits.remove(u);
					}
				}
				case unitHealth, unitBuildSpeed, unitMineSpeed, unitCost, unitDamage, blockHealth,
				     blockDamage, buildSpeed, rtsMinSquad, rtsMinWeight -> {
					Team team = p1.team();
					if (team != null) {
						float num = value.numf();
						switch (rule) {
							case buildSpeed ->
									team.rules().buildSpeedMultiplier = Mathf.clamp(num, 0.001f, 50f);
							case unitHealth ->
									team.rules().unitHealthMultiplier = Math.max(num, 0.001f);
							case unitBuildSpeed ->
									team.rules().unitBuildSpeedMultiplier = Mathf.clamp(num, 0f, 50f);
							case unitMineSpeed ->
									team.rules().unitMineSpeedMultiplier = Math.max(num, 0f);
							case unitCost -> team.rules().unitCostMultiplier = Math.max(num, 0f);
							case unitDamage ->
									team.rules().unitDamageMultiplier = Math.max(num, 0f);
							case blockHealth ->
									team.rules().blockHealthMultiplier = Math.max(num, 0.001f);
							case blockDamage ->
									team.rules().blockDamageMultiplier = Math.max(num, 0f);
