...
public class AsteroidGenerator extends BlankPlanetGenerator {
	...
	public void generate() {
		...
		//walls at insides
		pass((x, y) -> {
			if (floor == background || Ridged.noise2d(seed + 1, x, y, 4, 0.7f, 1f / 60f) > 0.45f || Mathf.within(x, y, sx, sy, 20 + Ridged.noise2d(seed, x, y, 3, 0.5f, 1f / 30f) * 6f))
				return;
		...
		pass((x, y) -> {
			...
			if (Math.abs(0.5f - noise(x, y + i * 999 - x * 1.5f, 2, 0.65, (60 + i * 2))) > 0.26f) {
		...
		//TODO maybe make this off by default everywhere
