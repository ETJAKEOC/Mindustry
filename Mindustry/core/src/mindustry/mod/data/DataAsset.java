...
import static mindustry.Vars.assetCache;
...
public abstract class DataAsset implements Comparable<DataAsset> {
	...
	public void setHash(byte[] value) {
		if (value.length != 32)
			throw new IllegalArgumentException("hash must be 32 bytes long: " + value.length);
		byteHash = value;
	...
	public Fi getCacheFileNoNull() {
		...
		if (file == null)
			throw new RuntimeException("Cache file for asset " + path + " not found!");
		return file;
	}
	...
	public void readFromZip(String path, Fi file) {
		if (isAlwaysEmbedded())
			throw new UnsupportedOperationException("Always-embedded assets need custom implementations for reading.");
		setPath(path);
