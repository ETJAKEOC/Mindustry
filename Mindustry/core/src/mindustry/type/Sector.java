...
public class Sector {
	...
	public String name() {
		...
		if (info.name != null) {
			String key = "sector." + info.name + ".name";
			if (Core.bundle != null && Core.bundle.has(key)) return Core.bundle.get(key);
			String shortName = info.name.contains("-") ? info.name.substring(info.name.lastIndexOf('-') + 1) : info.name;
			String altKey = "sector." + shortName + ".name";
			if (Core.bundle != null && Core.bundle.has(altKey)) return Core.bundle.get(altKey);
