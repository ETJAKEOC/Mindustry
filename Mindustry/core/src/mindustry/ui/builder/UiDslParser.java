...
public class UiDslParser {
	...
	public static TableBuilder parse(String source) {
		...
		p.skipWs();
		if (!p.atEnd())
			throw new SerializationException("Unexpected character '" + p.peek() + "' at line " + p.line(p.pos));
		return root;
	}
	...
	private void parseStatement(NodeBuilder<?> node) {
		skipWs();
		if (atEnd() || peek() == '"')
			throw new SerializationException("Expected identifier at line " + line(pos));
		String ident = readBareToken();
		if (ident.isEmpty())
			throw new SerializationException("Unexpected character '" + peek() + "' at line " + line(pos));
		...
		if (!atEnd() && peek() == '{') {
			pos++;
			if (!isNodeType)
				throw new SerializationException("Unknown node type '" + ident + "' at line " + line(pos));
			NodeBuilder<?> child = NodeBuilder.create(key);
			parseStatementsInto(child);
			expect('}');
			node.entries.add(new Entry(child.type, child));
	...
	private String parseString() {
		...
		if (atEnd())
			throw new SerializationException("Unterminated string starting at line " + line(pos));
		pos++; // closing quote
		return sb.toString();
	}
	...
	private void expect(char c) {
		if (atEnd() || peek() != c)
			throw new SerializationException("Expected '" + c + "', found '" + (atEnd() ? "<eof>" : peek()) + "' at line " + line(pos));
		pos++;
	}
