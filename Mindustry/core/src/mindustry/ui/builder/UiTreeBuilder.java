...
import mindustry.ui.builder.UiBuilder.Entry;
...
public class UiTreeBuilder {
	...
	private static void buildInto(Table table, Seq<Entry> entries, BuildContext ctx) {
		...
		for (Entry entry : entries) {
			if (entry.key == UiKey.row) {
				...
				if (cond != null && !evalCondition(cond))
					continue; // condition false - node is not added at all
	...
	private static Element addNode(UiKey type, NodeBuilder<?> node, BuildContext ctx) {
		return switch (type) {
			...
			case stack -> {
				...
				for (Entry entry : node.entries) {
					if (entry.value instanceof NodeBuilder<?> child) {
						...
						if (element == null)
							continue; // unknown/unsupported node - skip for forward compat
						...
						if (colorStr != null)
							element.setColor(Strings.parseColor(colorStr, Color.white));
			...
			case image -> {
				...
				if (region.startsWith(DataImagePacker.serverRegionPrefix))
					ctx.images.get(region, Seq::new).add(image);
			...
			case button -> {
				...
				if (icon != null) {
					...
					// track server-streamed images so that they can be reloaded when streaming finishes
					if (icon.startsWith(DataImagePacker.serverRegionPrefix))
						ctx.images.get(icon, Seq::new).add(iconImage);
			...
			case imageButton -> {
				...
				// track server-streamed images so that they can be reloaded when streaming finishes
				if (icon != null && icon.startsWith(DataImagePacker.serverRegionPrefix))
					ctx.images.get(icon, Seq::new).add(btn.getImage());
			...
			case buttonTable -> {
				...
				if (!applyStyle(node, TextButtonStyle.class, btn::setStyle))
					applyStyle(node, ImageButtonStyle.class, btn::setStyle);
				Float margin = node.num(UiKey.margin);
	...
	private static void fireResult(BuildContext ctx, String result) {
		...
		for (var entry : ctx.idElements) {
			...
			else if (el instanceof Stack stack && stack.getChildren().size > 0 && stack.getChildren().get(0) instanceof Slider slider)
				res.values.put(entry.key, slider.getValue());
			...
			else if (el instanceof Button b && b.getStyle().checked != null)
				res.values.put(entry.key, b.isChecked());
		}
	...
	private static void applyCellProp(Cell<?> cell, UiKey key, Object value) {
		if (value == Boolean.TRUE) {
			switch (key) {
				...
				case disabled -> {
					if (cell.get() instanceof Disableable d) d.setDisabled(true);
				}
				default -> {
				}
			}
