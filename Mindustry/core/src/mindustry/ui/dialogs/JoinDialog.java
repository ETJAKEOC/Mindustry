...
import static mindustry.Vars.cachedServers;
import static mindustry.Vars.defaultServers;
import static mindustry.Vars.fetchedServers;
import static mindustry.Vars.loadedServerCache;
...
import static mindustry.Vars.serverCacheFile;
import static mindustry.Vars.serverJsonBeURLs;
import static mindustry.Vars.serverJsonURLs;
...
public class JoinDialog extends BaseDialog {
			...
			for (String address : group.addresses) {
				...
				net.pingHost(resaddress, resport, res -> {
					...
					if (!serverSearch.isEmpty() && !(group.name.toLowerCase().contains(serverSearch)
							...
							|| (res.modeName != null && Strings.stripColors(res.modeName.toLowerCase()).contains(serverSearch))))
						return;
	...
	void addCommunityHost(Host host, Table container) {
		...
		buildServer(host, button[0].table(t -> {
		}).grow().get(), false, false);
