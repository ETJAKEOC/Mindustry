...
import static mindustry.Vars.modGuideURL;
...
public class ModsDialog extends BaseDialog {
		...
		browser.getModList(list -> {
			...
			for (var entry : list) {
				...
				if (mod != null) {
					...
					if (Strings.checkNewerSemver(entry.version, mod.meta.version))
						withUpdates.add(mod);
				}
			}
	...
	void setup() {
		...
		if (!mods.list().isEmpty() || !mods.preinstalledMods.isEmpty()) {
			...
			Cons<String> rebuild = query -> {
				...
				boolean preAny = false;
				for (int i = 0; i < mods.preinstalledMods.size; i++) {
					var pre = mods.preinstalledMods.get(i);
					if (query.isEmpty() || pre.matches(query)) {
						any = true;
						if (!preAny) {
							preAny = true;
							cont.add("@mod.preinstalled.title").color(Pal.accent).left().padLeft(4f).row();
							cont.image().growX().height(3f).pad(4f).color(Pal.accent).row();
						}
						...
						cont.button(t -> {
							...
							t.table(title1 -> {
								...
								title1.table(text -> {
									text.add("[accent]" + pre.displayName + "\n[lightgray]" + pre.description)
											.ellipsis(true).top().width(300f).growX().left();

									text.row();
									text.labelWrap("[accent]" + Core.bundle.get("mod.preinstalled.cannotremove")).growX().row();
								}).top().growX();
							...
							t.table(right -> {
								right.right();
								right.add(new Image(Icon.lock)).size(32f).color(Pal.accent);
							}).growX().right().padRight(-8f).padTop(-8f);
						}, Styles.grayt, () -> {
							BaseDialog dialog = new BaseDialog(pre.displayName);
							dialog.cont.pane(p -> p.add(pre.description + "\n\n[accent]" + Core.bundle.get("editor.author") + "[lightgray] " + pre.author + "\n\n[accent]" + Core.bundle.get("mod.preinstalled.details"))
									.width(mobile ? 400f : 500f).wrap().pad(4f).labelAlign(Align.center, Align.left)).grow();
							dialog.addCloseButton();
							dialog.show();
						}).size(w, h).growX().pad(4f).padTop(4f).row();
					}
				}

				if (preAny && !mods.list().isEmpty()) {
					cont.image().growX().height(2f).pad(12f).color(Pal.gray).row();
				}
	...
	private void showMod(LoadedMod mod) {
		...
		if (mod.getRepo() != null) {
			...
			if (showImport)
				modDetails.buttons.button("@mods.browser.reinstall", Icon.download, () -> viewReleases(mod.getRepo(), mod.isJava(), true));
		}
			...
			modDetails.cont.button("@mods.viewcontent", Icon.book, () -> {
				...
				d.cont.pane(cs -> {
					...
					for (UnlockableContent c : all) {
						...
						if (++i % (int) Math.min(Core.graphics.getWidth() / Scl.scl(110), 14) == 0)
							cs.row();
					}
				}).grow();
				d.addCloseButton();
	...
	public void githubImportMod(String repo, boolean isJava, @Nullable String release, boolean forceEnable) {
		if (mods.isPreinstalled(repo)) {
			ui.showInfo("@mod.preinstalled.cannotinstall");
			return;
		}
