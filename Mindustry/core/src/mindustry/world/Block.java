...
public class Block extends UnlockableContent implements Senseable {
	...
	public void setStats() {
		...
		if (hasItems) {
			...
			else if (instantTransfer || itemCapacity == 0)
				stats.add(Stat.itemCapacity, Core.bundle.format("none") + ". " + Core.bundle.format("unabletoclog"));
		}
	}
	...
	public void createIcons(MultiPacker packer) {
		...
		if (gen.length > 0 && gen[0] != null && gen[0].found()) {
			if (gen.length > 1) {
				...
				if (gen[0] != null)
					packer.add(PageType.main, "block-" + name + "-full", packer.get(gen[0]));
			}
		}
