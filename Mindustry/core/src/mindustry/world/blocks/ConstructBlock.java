...
public class ConstructBlock extends Block {
	...
	public static ConstructBlock get(int size) {
		if (size > maxBlockSize)
			throw new IllegalArgumentException("No. Don't place ConstructBlocks of size greater than " + maxBlockSize);
	...
	public static void deconstructFinish(Tile tile, Block block, Unit builder) {
		...
		if (!headless && fogControl.isVisibleTile(Vars.player.team(), tile.x, tile.y)) {
			...
			if (shouldPlay())
				block.breakSound.at(tile, block.breakPitchChange ? calcPitch(false) : 1f);
		}
	...
	public static void constructFinish(Tile tile, Block block, @Nullable Unit builder, byte rotation, Team team, Object config) {
		...
		if (fogControl.isVisibleTile(team, tile.x, tile.y)) {
			...
			if (shouldPlay())
				block.placeSound.at(tile, block.placePitchChange ? calcPitch(true) : 1f);
		}
	...
	public static void playRepairSound(Team team, Tile tile) {
		if (!headless && shouldPlay() && fogControl.isVisibleTile(team, tile.x, tile.y))
			Sounds.blockRepair.at(tile, calcPitch(true));
	}
