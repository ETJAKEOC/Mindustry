...
public class LandingPad extends Block {
	...
	public LandingPad(String name) {
		...
		config(Item.class, (LandingPadBuild build, Item item) -> {
			if (!build.accessible() || !item.isOnPlanet(state.getPlanet()) || !item.unlockedNow())
				return;
		...
		public void display(Table table) {
			...
			table.label(() -> {
				...
				for (var otherSector : state.getPlanet().sectors) {
					if (otherSector == state.getSector() || !otherSector.hasBase() || otherSector.info.destination != state.getSector())
						continue;
