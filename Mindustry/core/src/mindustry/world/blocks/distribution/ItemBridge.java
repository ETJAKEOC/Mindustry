...
public class ItemBridge extends Block {
	...
	public boolean linkValid(Tile tile, Tile other, boolean checkDouble) {
		if (other == null || tile == null || !positionsValid(tile.x, tile.y, other.x, other.y, linkSameType ? range
				: Math.max(tile.block() instanceof ItemBridge bt ? bt.range : range, other.block() instanceof ItemBridge bo ? bo.range : range)))
			return false;
		if (!linkSameType) {
			//false if at least 1 module isn't  shared
			if (!(tile.block().hasItems && other.block().hasItems) && !(tile.block().hasLiquids && other.block().hasLiquids))
				return false;
		}
