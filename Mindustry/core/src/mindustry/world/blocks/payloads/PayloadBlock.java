...
public class PayloadBlock extends Block {
	...
	protected TextureRegion findFactoryRegion(String suf) {
		...
		if (!region.found() && minfo.mod != null)
			region = Core.atlas.find(minfo.mod.name + "-factory" + suf + "-" + size + regionSuffix);
	...
	public class PayloadBlockBuild<T extends Payload> extends Building {
		...
		public boolean canControlSelect(Unit unit) {
			if (unit.spawnedByCore || !unit.type.allowedInPayloads || this.payload != null || !acceptUnitPayload(unit))
				return false;
