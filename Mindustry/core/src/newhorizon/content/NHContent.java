...
public class NHContent extends Content {
    ...
    Texture loadTex(String name, Cons<Texture> modifier) {
        String filename = "textures/" + name + (name.endsWith(".png") ? "" : ".png");
        Fi file = Vars.tree.get(filename);
        if ((file == null || !file.exists()) && NewHorizon.MOD != null && NewHorizon.MOD.root != null) {
            file = NewHorizon.MOD.root.child(filename);
        }
