...
//			}
		...
		public void drawConfigure() {
			...
			if (builds.any())
				DrawFunc.overlayText(Core.bundle.format("mod.ui.participants", builds.size), targetVec.x, targetVec.y, tilesize * 2f, Pal.accent, true);
		}
		...
		public void commandAll(Vec2 pos) {
			...
			if (!Vars.headless && participantsTmp.any()) {
				if (team != Vars.player.team())
					NHUIFunc.showToast(Icon.warning.getRegion(), "[#ff7b69]Caution: []Attack " + (int) (pos.x / 8) + ", " + (int) (pos.y / 8), NHSounds.uiAlert1, team.color);
