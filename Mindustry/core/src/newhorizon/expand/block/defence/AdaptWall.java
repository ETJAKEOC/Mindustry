...
public class AdaptWall extends Wall {
	...
	public void load() {
		super.load();
		atlasRegion = SpriteUtil.splitRegionArray(SpriteUtil.findRegion(name + "-tiled"), 32, 32);
		TextureRegion innerRegion = SpriteUtil.findRegion(name + "-inner-tiled");
		if (innerRegion.found()) {
			innerAtlasRegions = SpriteUtil.splitRegionArray(innerRegion, 32, 32, 0, SpriteUtil.ATLAS_INDEX_4_4_VANILLA);
			innerFound = true;
		}
	}
	...
	public class AdaptWallBuild extends Building {
		...
		public void draw() {
			...
			if (atlasRegion != null && atlasRegion.length > 0) {
				TextureRegion reg = drawIndex < atlasRegion.length ? atlasRegion[drawIndex] : atlasRegion[0];
				Draw.rect(reg, x, y);
			} else {
				super.draw();
			}
			if (innerFound && innerAtlasRegions != null && drawIndex == 13 && drawInnerIndex < innerAtlasRegions.length) {
