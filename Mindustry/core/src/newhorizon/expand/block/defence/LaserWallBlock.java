...
public class LaserWallBlock extends Block {
	...
	public void drawPlace(int x, int y, int rotation, boolean valid) {
		...
		if (selected == null || !(selected.block instanceof LaserWallBlock) || !selected.within(xB, yB, range))
			return;
	...
	public static class Shooter extends BulletType {
		...
		public void update(Bullet bullet) {
			...
			if (bullet.timer(0, 5f))
				Damage.collideLine(bullet, bullet.team, bullet.x, bullet.y, bullet.rotation(), bullet.dst(build), true, false);
			...
			if (Mathf.chanceDelta(0.075))
				PosLightning.createEffect(bullet, build, lightningColor, 0, 0);
		}
	...
	public class LaserWallBuild extends Building implements Linkablec {
		...
		public void updateTile() {
			...
			if (power.status > 0.5f && canActivate())
