...
public class AdaptConveyor extends ArmoredConveyor {
	...
	public void load() {
		...
		edgeRegions = SpriteUtil.splitRegionArray(name + "-edge", 32, 32, 0, SpriteUtil.ATLAS_INDEX_4_4);
		lightRegions = SpriteUtil.splitRegionArray(name + "-light", 32, 32, 0, SpriteUtil.ATLAS_INDEX_4_4);
		pulseRegions = SpriteUtil.splitRegionArray(name + "-pulse", 32, 32, 0, SpriteUtil.ATLAS_INDEX_4_4);
		arrowRegions = SpriteUtil.splitRegionArray(name + "-arrow", 32, 32, 1);
	}
	...
	public class AdaptConveyorBuild extends ConveyorBuild {
		...
		public void draw() {
			...
			if (pulseRegions != null && pulseRegions.length > 0) {
				Draw.rect(pulseRegions[Math.min(drawIndex, pulseRegions.length - 1)], x, y);
			}

			Draw.z(Layer.block - 0.2f);
			if (arrowRegions != null && arrowRegions.length > 0) {
				int frame = conveyorFrame() % arrowRegions.length;
				Draw.rect(arrowRegions[frame], x, y, tilesize * blendsclx, tilesize * blendscly, rotation * 90);

				boolean backDraw = true;
				int sideFrame = (conveyorFrame() + 16) % arrowRegions.length;
				if (blends(this, right())) {
					Draw.rect(arrowRegions[sideFrame], x, y, rotdeg() + 90);
					backDraw = false;
				}
				if (blends(this, back())) {
					Draw.rect(arrowRegions[sideFrame], x, y, rotdeg());
					backDraw = false;
				}
				if (blends(this, left())) {
					Draw.rect(arrowRegions[sideFrame], x, y, rotdeg() - 90);
			...
			Draw.color();
			Draw.reset();
			if (edgeRegions != null && edgeRegions.length > 0) {
				Draw.rect(edgeRegions[Math.min(drawIndex, edgeRegions.length - 1)], x, y);
			}

			if (lightRegions != null && lightRegions.length > 0) {
				Draw.color(team.color, Color.white, 0.25f);
				Draw.alpha(0.4f);
				Draw.rect(lightRegions[Math.min(drawIndex, lightRegions.length - 1)], x, y);
				Draw.alpha(1f);
				Draw.color();
			}
