...
public class AdaptStackConveyor extends StackConveyor {
    ...
    public void drawPlanRegion(BuildPlan plan, Eachable<BuildPlan> list) {
        ...
        TextureRegion topReg = (topRegions != null && topRegions.length > 0) ? topRegions[Math.min(plan.rotation, topRegions.length - 1)] : region;
        Draw.rect(onlyCarry ? regions[0] : topReg, plan.drawx(), plan.drawy(), onlyCarry ? plan.rotation * 90 : 0);
    ...
    public void load() {
        super.load();
        edge2Region = SpriteUtil.findRegion(name + "-edge-d");
        topRegions = SpriteUtil.splitRegionArray(name + "-top", 32, 32, 1);
    }
        ...
        public void draw() {
            ...
            if (onlyCarry) {
                ...
                if (topRegions != null && topRegions.length > 0) {
                    Draw.rect(topRegions[Math.min(rotation, topRegions.length - 1)], x, y);
                } else {
