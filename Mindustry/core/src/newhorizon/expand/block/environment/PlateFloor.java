...
public class PlateFloor extends Floor {
	...
	public TextureRegion getDrawRegion(Tile tile) {
		if (plateRegions[getSize(tile)] == null || plateRegions[getSize(tile)].length <= getType(tile))
			return region;
		TextureRegion[][] variants = plateRegions[getSize(tile)];
		if (variants[getType(tile)] == null || variants[getType(tile)].length <= tile.floorData)
			return region;
