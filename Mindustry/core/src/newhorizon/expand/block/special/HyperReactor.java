	...
	public void init() {
		...
		destroyed = new EffectBulletType(600f) {
			...
			public void update(Bullet b) {
				...
				if (b.timer(3, 8))
					PosLightning.createRange(b, b, Team.derelict, lightningRange * 2f, 255, effectColor, true, lightningDamage, subNum + Mathf.random(subNumRand), PosLightning.WIDTH, updateLightning + Mathf.random(updateLightningRand), point -> {
		...
		public void updateTile() {
			...
			if (efficiency > 0) {
				...
				if (Mathf.equal(warmup, 1.0F, 0.0015F)) {
					...
					warmup = Mathf.lerpDelta(warmup, 1, warmupSpeed + (Mathf.num(Vars.net.active()) * 0.025f));
			} else {
			...
			if (warmup > minWarmup) {
				for (int i : Mathf.signs) {
					if (Mathf.chance(warmup / updateEffectDiv))
						updateEffect.at(x + i * Mathf.random(effectCircleSize), y + i * Mathf.random(effectCircleSize), updateEffectSize * warmup, effectColor);
				}
				...
				if (Mathf.chanceDelta(warmup / updateEffectDiv * 1.5f))
					workEffect.at(x, y, updateEffectSize * 3f * warmup, effectColor);
