...
public class JumpGate extends Block {
	...
	public class JumpGateBuild extends Building implements Ranged {
		...
		public boolean hasConsume(UnitRecipe set, int num) {
			if (set == null || cheating() || (!state.rules.pvp && team == state.rules.waveTeam))
				return true;
		...
		public void moveQueue(int from, int to) {
			if (from < 0 || to < 0 || from >= buildQueue.size || to >= buildQueue.size || from == to)
				return;
		...
		public void buildConfiguration(Table table) {
						...
						Cons<Table> fillBuilding = host -> {
							...
							host.stack(
									...
									new Table(row -> {
										...
										if (loop)
											row.image(Icon.refresh).size(ui(24f)).padRight(ui(6f));
							...
							host.stack(
									...
									new Table(row -> {
										...
										if (loop)
											row.image(Icon.refresh).size(ui(24f)).padRight(ui(6f));
										if (jam)
											row.add("[red]" + Core.bundle.get("spawn-error")).right();
			...
			dialog.keyDown(c -> {
				if (c == KeyCode.left)
					selectNum = Mathf.clamp(--selectNum, 1, Mathf.clamp(Units.getCap(team), 1, maxSpawnCount));
				if (c == KeyCode.right)
					selectNum = Mathf.clamp(++selectNum, 1, Mathf.clamp(Units.getCap(team), 1, maxSpawnCount));
				if (c == KeyCode.enter && selectID >= 0)
					configure(IntSeq.with(0, selectID, selectNum));
			});
