...
public class DefaultSpecialEvent {
	...
	public static void readState(DataInput in, boolean readLoops) throws IOException {
		...
		if (count < 0 || count > 1024)
			throw new IOException("Invalid New Horizon special event state count: " + count);
		...
		if (loops < 0 || loops > 1024)
			throw new IOException("Invalid New Horizon special event loop state count: " + loops);
	...
	public static void update() {
		...
		if (state.rules.editor || state.rules.mode() == Gamemode.sandbox || state.rules.mode() == Gamemode.pvp)
			return;
	...
	private static boolean hasActiveAction(ActionBus bus, int eventId) {
		...
		if (bus.current instanceof EventInterventionAction action && action.eventId == eventId && !action.complete())
			return true;
		for (newhorizon.expand.logic.components.Action action : bus.queue) {
			if (action instanceof EventInterventionAction intervention && intervention.eventId == eventId && !intervention.complete())
				return true;
		}
		return false;
	}
