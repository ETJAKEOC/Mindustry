...
public class EventSaveData implements SaveFileReader.CustomChunk {
	...
	public void readSnapshot(DataInput stream) throws IOException {
		...
		if (count < 0 || count > 256)
			throw new IOException("Invalid New Horizon event count: " + count);
	...
	private void collectAction(Action action, Seq<SavedEvent> events) {
		...
		if (action instanceof EventRaidAction raid) {
			if (!raid.presentationOnly && !contains(events, raid))
				events.add(SavedEvent.from(raid));
		} else if (action instanceof EventInterventionAction intervention) {
			if (!intervention.presentationOnly && !contains(events, intervention))
				events.add(SavedEvent.from(intervention));
		...
		private static SavedEvent from(EventInterventionAction action) {
			...
			for (EventInterventionAction.UnitEntry entry : action.units) {
				if (entry != null && entry.type != null)
					saved.units.add(new int[]{entry.type.id, entry.count});
			}
			return saved;
		}
		...
		private static SavedEvent read(DataInput in) throws IOException {
			...
			if (saved.type == RAID) {
				...
				if (count < 0 || count > 1024)
					throw new IOException("Invalid New Horizon unit count: " + count);
				for (int i = 0; i < count; i++)
					saved.units.add(new int[]{in.readInt(), in.readInt()});
				...
				int count = in.readInt();
				if (count < 0 || count > 1024)
					throw new IOException("Invalid New Horizon special unit count: " + count);
		...
		private Action create() {
			...
			if (type == INTERVENTION) {
				...
				for (int[] unit : units) {
					...
					if (type != null && unit[1] > 0)
						action.units.add(new EventInterventionAction.UnitEntry(type, unit[1]));
				}
	...
	private static final class SavedSpecialUnit {
		...
		private static SavedSpecialUnit read(DataInput in) throws IOException {
			...
			if (statusCount < 0 || statusCount > 1024)
				throw new IOException("Invalid New Horizon special status count: " + statusCount);
			for (int i = 0; i < statusCount; i++) saved.statuses.add(SavedSpecialStatus.read(in));
			int itemCount = in.readInt();
			if (itemCount < 0 || itemCount > 1024)
				throw new IOException("Invalid New Horizon special item count: " + itemCount);
			for (int i = 0; i < itemCount; i++)
				saved.items.add(new int[]{in.readInt(), in.readInt()});
			return saved;
		}
		...
		private SpecialEvent.UnitSpec create() {
			...
			for (SavedSpecialStatus saved : statuses) {
				...
				if (effect != null)
					unit.statuses.add(new SpecialEvent.StatusSpec(effect, saved.duration));
			}
			for (int[] item : items) {
				Item contentItem = SavedEvent.item(item[0]);
				if (contentItem != null && item[1] > 0)
					unit.items.add(new ItemStack(contentItem, item[1]));
			}
			return unit;
		}
	}
