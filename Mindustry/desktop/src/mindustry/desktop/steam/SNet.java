...
import steamworks.SteamFriends;
import steamworks.SteamFriendsCallback;
import steamworks.SteamID;
import steamworks.SteamMatchmaking;
import steamworks.SteamMatchmaking.ChatMemberStateChange;
import steamworks.SteamMatchmaking.ChatRoomEnterResponse;
import steamworks.SteamMatchmaking.LobbyDistanceFilter;
import steamworks.SteamMatchmaking.LobbyType;
import steamworks.SteamMatchmakingCallback;
import steamworks.SteamNetworkingConfigValue;
import steamworks.SteamNetworkingSockets;
import steamworks.SteamNetworkingSockets.Connection;
import steamworks.SteamNetworkingSockets.ConnectionState;
import steamworks.SteamNetworkingSockets.SendFlags;
import steamworks.SteamNetworkingSockets.Socket;
import steamworks.SteamNetworkingSocketsCallback;
...
public class SNet implements SteamNetworkingSocketsCallback, SteamMatchmakingCallback, SteamFriendsCallback, NetProvider {
	...
	public void connectClient(String ip, int port, Runnable success) throws IOException {
		...
		if (ip.startsWith("steam:")) {
			...
			try {
				...
				if (!serverID.isValid())
					throw new IOException("Invalid Steam ID structure: " + server);
	...
	void updateBans(String changed) {
		if (changed != null && !changed.startsWith("steam:"))
			return; //hacky way to ignore non-steam ids
	...
	public void onLobbyChatUpdate(SteamID lobby, SteamID who, SteamID changer, ChatMemberStateChange change) {
		...
		if (net.server() && change == ChatMemberStateChange.Entered && SteamAdmin.isAdmin("steam:" + who.getAccountID()))
			SteamAdmin.fetch(true); //fetch on admin join
		...
		if (lobbyDoneCallback != null) {
			...
			for (int i = 0; i < matches; i++) {
				try {
					...
					if (mode == null || mode.isEmpty() || (Version.build != -1 && Strings.parseInt(smat.getLobbyData(lobby, "version"), -1) != Version.build))
						continue;
